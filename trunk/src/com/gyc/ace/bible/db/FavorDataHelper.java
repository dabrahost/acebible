...
public class FavorDataHelper extends SQLiteOpenHelper {

	private static final String TABLE_NAME = "BibleMarks";
	private Context mContext;

	/** Constructor */
	public FavorDataHelper(Context context) {
		super(context, "BibleMarks", null, 1);
		this.mContext = context;
	}

	public FavorDataHelper(Context context, String name, CursorFactory factory,
			int version) {
	...
	public void delete(long task_id) {
		...
		try {
			getWritableDatabase().delete(FavorDataHelper.TABLE_NAME, "id=?",
					whereArgs);
		} catch (SQLException e) {
	...
	public void delete4SQL(long task_id) {
		String sql = String.format("DELETE FROM " + FavorDataHelper.TABLE_NAME
				+ " WHERE id = '%d' ", task_id);
		try {
